...
type Handler struct{ repo *repositories.InventoryRepo }

func New(r *repositories.InventoryRepo) *Handler { return &Handler{repo: r} }
...
func (h *Handler) List(c *fiber.Ctx) error {
	ctx, cancel := ctx10s()
	defer cancel()
	list, err := h.repo.FindBySchool(ctx, c.Query("schoolId"), c.Query("category"))
	if err != nil {
